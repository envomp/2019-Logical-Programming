...
get_len([], 0).
get_len([Head|Tail], Count) :- get_len(Tail, Sum), Count is Sum + 1.
...
extinction(Pray, DeadSpieces, DeadCount) :- eats(Predator, Pray), extinction(Predator, DeadLast, Count),count_terminals(Pray, DeadLeaves, DeadLeavesCount), append(DeadLast, DeadLeaves, DeadSpieces), DeadCount is Count + DeadLeavesCount.
...
find_most_sensitive_species(Spieces, DeadSpieces, DeadCount) :- cycle, !, best(Spieces, DeadSpieces, DeadCount).
