...
% simple eat
test_board_2(
	     game_board(l(0, b, 0, 1, 0, 1, 0, 1), l(1, 0, w, 0, 1, 0, 1, 0), l(0, 1, 0, 1, 0, 1, 0, 1), l(1, 0, 1, 0, 1, 0, 1, 0), l(0, 1, 0, b, 0, b, 0, 1), l(1, 0, 1, 0, w, 0, w, 0), l(0, 1, 0, 1, 0, 1, 0, 1), l(1, 0, 1, 0, 1, 0, 1, 0))
	     ).
...
board([fig(man,0,6),fig(man,1,7),fig(man,1,5),fig(man,2,6),fig(man,3,7),fig(man,3,5),fig(man,4,6),fig(man,5,7),fig(man,5,5),fig(man,6,6),fig(man,7,7),fig(man,7,5)],[fig(man,0,0),fig(man,0,2),fig(man,1,1),fig(man,2,0),fig(man,2,2),fig(man,3,1),fig(man,4,0),fig(man,4
,2),fig(man,5,1),fig(man,6,0),fig(man,6,2),fig(man,7,1)])

iaib185787(MyColor, _, _):-
    make_board(Board),
    ((MyColor = 1,  make_play(white, Board)) ; (MyColor = 0,  make_play(black, Board))).

iaib185787(_, _, _).

make_play(Color, Board) :-
    alphabeta(Color, -1000, 1000, Board, NextMove, Eval, 0), write(NextMove).
...
make_board(Board) :-
    ...
    make_list(Row7, 2),
    make_list(Row8, 1),
    Board = game_board(Row1,Row2,Row3,Row4,Row5,Row6,Row7,Row8).

make_list(List, Row) :-
    element(Element1, Row, 1),
    element(Element2, Row, 2),
    element(Element3, Row, 3),
    element(Element4, Row, 4),
    element(Element5, Row, 5),
    element(Element6, Row, 6),
    element(Element7, Row, 7),
    element(Element8, Row, 8),
    List = l(Element1,Element2,Element3,Element4,Element5,Element6,Element7,Element8).

element(w, X, Y) :- ruut(X, Y, C), C = 1.
element(wq, X, Y) :-  ruut(X, Y, C), C = 10.
element(b, X, Y) :- ruut(X, Y, C), C = 2.
element(bq, X, Y) :- ruut(X, Y, C), C = 20.
element(1, X, Y) :- ruut(X, Y, C), C = 0.
element(0, X, Y) :- not(ruut(X, Y, C)).
